# Änderungshistorie - MII Implementation Guide Microbiology v2027.0.0-ballot.rc1
...
* Die Profile wurden Artefakt für Artefakt gegen das europäische Terminologie-Workbook, das begleitende Whitepaper und den realen Ergebniskatalog eines deutschen Labors für Gram- und Säurefest-Präparate abgeglichen. Die Ableitungsregel, der das Modul seit 2027.0.0-alpha.1 folgt — ein Profil folgt der Fragestellung und dem Ergebnistyp, nicht dem Verfahren —, wurde von den Quellen bestätigt und nicht revidiert: Das Whitepaper führt sie als Grundsatz, sein Kulturkapitel leitet daraus dieselbe Trennung von Kultur und Identifizierung ab, und sein Empfindlichkeitskapitel trennt Messwert und Kategorie auf dieselbe Weise.
* Proben haben jetzt ein Profil. Das Modul verlangt `Observation.specimen` seit 2027.0.0-alpha.1 für jede Untersuchung, gab der Referenz aber kein Zielprofil, sodass jedes beliebige Specimen genügte. `mii-pr-mikrobio-probe` leitet von dem Bioproben-Basisprofil ab, das genau dafür existiert, und jedes Untersuchungsprofil verweist nun darauf.
...
* Die Mikroskopie kann eine Morphologie zusammen mit ihrer Menge tragen. „Wenig grampositive Kokken" braucht zwei Aussagen, `value[x]` trägt eine — die Menge steht deshalb in einer Komponente. Ihr Code stammt aus einem Interims-CodeSystem dieses Moduls, bis der vom europäischen Modell angeforderte LOINC-Code existiert.
...
* Die Serologie ist von der Specimen-Postkoordination ausgenommen. Das europäische Whitepaper erlaubt dort ein präkoordiniertes Material, weil nur wenige Materialien vorkommen und überwiegend Serum. Die Ausnahme betrifft allein den Untersuchungscode; eine Specimen-Ressource wird weiter erwartet.
* Empfindlichkeitsergebnisse dürfen als Kategorie angegeben werden, wenn kein Messwert vorliegt — das europäische Modell legt es ausdrücklich so fest. Zwei Invarianten halten das eindeutig, eine dritte verlangt eine Interpretation, wo ein Messwert vorliegt.
* Punkte, die die Quellen offen lassen oder in denen dieser Leitfaden von ihnen abweicht, stehen jetzt als **Ballotfragen** im Leitfaden statt stillschweigend entschieden zu sein. Sechs sind offen, jede auf der Seite, zu der sie gehört, und gesammelt auf der Startseite. Ihre Antworten hängen davon ab, was Standorte liefern können — deshalb gehen sie ins Ballot und werden nicht hier entschieden.
...
* Die diagnostische Kette ist jetzt dort dokumentiert, wo sie verwendet wird. `triggeredBy`, `derivedFrom` und `hasMember` waren allein auf der Seite zur Profilauswahl beschrieben; jedes an einer Kette beteiligte Profil nennt nun seine eigene Stellung darin, die Empfindlichkeit trägt das Panel-Muster für ein Antibiogramm, und das Befundprofil hält fest, dass es die Schritte bündelt, während die Observations selbst die Beziehungen zwischen ihnen tragen.
...
| `mii-pr-mikrobio-spezifische-mikroskopie` | neu | bisher nicht abbildbar | Gezielte Mikroskopie: Ein ordinaler LOINC-Code benennt Objekt und Färbung,`value[x]`trägt die semiquantitative Stufe als`CodeableConcept`oder eine Zählung je Gesichtsfeld als`Quantity`oder`Range` | Säurefeste Färbungen, Wirtszellzählungen und Clue Cells werden kodierbar; sie hatten vorher kein Profil | Solche Befunde von der allgemeinen Mikroskopie hierher umstellen, wo die Menge in die`interpretation`gezwungen werden musste |
| `mii-pr-mikrobio-mikroskopie` | inhaltlich aktualisiert | `code`fest auf`105059-0`; keine Komponente; Titel „Mikroskopie" | `code`aus`mii-vs-mikrobio-allgemeine-mikroskopie-tests-loinc`(extensible), das auch`664-3`zulässt; neue`component[menge]`für die semiquantitative Menge;`method`benennt die Färbungspräferenz; Titel „Allgemeine Mikroskopie" | Der feste Code wird nicht mehr erzwungen; Morphologie und Menge lassen sich zusammen senden | Keine Migration nötig.`105059-0`bleibt bevorzugt;`664-3`hält`Observation.method`frei, das`0..1`ist und Färbung und Mikroskopieverfahren nicht beide tragen kann |
| `mii-pr-mikrobio-allgemeine-kultur` | inhaltlich aktualisiert | `code`fest auf`11475-1` | `code`aus`mii-vs-mikrobio-allgemeine-kultur-tests-loinc`(extensible), das auch den methodenneutralen`41852-5`zulässt; neues Invariant`allgemeine-kultur-method-with-neutral-code`verlangt für diesen Code`Observation.method` | Eine Lockerung: Jede bisher gültige Instanz bleibt gültig. Bei`41852-5`wird die Methode zur Pflicht, weil erst sie die Untersuchung als Kultur ausweist | Keine Migration nötig.`11475-1`bleibt empfohlen: Es benennt das Verfahren selbst und unterscheidet die Kultur damit von der allgemeinen Bestimmung — eine Frage, die die europäischen Quellen nicht adressieren |
| `mii-pr-mikrobio-empfindlichkeit` | inhaltlich aktualisiert | `value[x]`nur`Quantity`;`method`ungebunden | `value[x]`nimmt`Quantity`oder`CodeableConcept`;`method`an`mii-vs-mikrobio-empfindlichkeit-methode-snomed`gebunden (extensible); drei Invarianten: Eine Kategorie verlangt eine Interpretation, beide müssen übereinstimmen, und ein Messwert sollte eine Interpretation tragen (Warnung) | Ein Labor, das nur SIR berichtet, muss`value[x]`nicht mehr leer lassen. Methodencodes sind jetzt beschränkt, wo sie frei waren | Bei jedem kategorialen Wert eine Interpretation angeben; dort hängt die Norm, und eine Kategorie ohne sie sagt nichts |
| `mii-pr-mikrobio-voraussichtliche-empfindlichkeit` | inhaltlich aktualisiert | Description: abgeleitet aus Resistenzmechanismen, Genen oder Proteinen | Description: abgeleitet aus genotypischen Resistenznachweisen;`code`benennt die Abgrenzung zur gemessenen Empfindlichkeit | Nur Dokumentation, keine Constraint-Änderung | Die weitere Formulierung versprach mehr, als LOINC hergibt: Am 2026-09-10 gemessen gibt es 99`[Susceptibility]`-Codes mit Methode Genotyping und keinen mit einer Immunoassay-Methode — eine proteinbasierte Vorhersage ist nicht kodierbar |
| `mii-pr-mikrobio-diagnostic-report` | inhaltlich aktualisiert | `conclusion`unerwähnt;`result`nahm die spezifische Mikroskopie nicht auf | `conclusion`als tragendes Element serologischer Befunde dokumentiert;`mii-pr-mikrobio-spezifische-mikroskopie`als zulässiges Ziel ergänzt | Befunde können gezielte Mikroskopie referenzieren; die Konklusion hat einen dokumentierten Zweck | keine Migration nötig |
...
| `mii-pr-mikrobio-allgemeine-bestimmung` | inhaltlich aktualisiert | Titel „MII PR Mikrobio Allgemeine Bestimmung" | Titel „MII PR Mikrobio Allgemeine Bestimmung (Identifizierung)" | Nur der Titel; Id und Canonical unverändert | „Bestimmung" ist im kulturellen Kontext mehrdeutig, wo das Profil faktisch die MALDI-TOF-Speziesidentifizierung trägt |
...
| `mii-pr-mikrobio-diagnostic-report` | inhaltlich aktualisiert | `result`verwies auf 18 Untersuchungsprofile;`category[mibi-sub-category]`war`0..1` | `mii-pr-mikrobio-spezifische-kultur`als zulässiges Ziel ergänzt;`category[mibi-sub-category]`auf`0..*`erweitert | Befunde können auf spezifische Kulturen verweisen; Befunde, die mehrere Studientypen umfassen — z. B. bakteriologisch und mykologisch — können alle zutreffenden Befundtypen angeben | keine Migration nötig; bisher übermittelte Einzelwerte bleiben gültig |
...
| | | | | | |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `mii-cs-mikrobio-mikroskopie-komponenten`(CodeSystem) | neu | - | Ein Interimscode,`semiquantitative-menge`, für die Mikroskopie-Komponente | Macht den Fall Morphologie-plus-Menge abbildbar | INTERIM. Zu ersetzen durch den LOINC-Code, den das europäische Modell anfordert (Blatt „Microscopy", Zeile 13); das CodeSystem wird dann retired. Vermerkt in docs/release-checklist.md |
...
| `mii-vs-mikrobio-allgemeine-mikroskopie-tests-loinc` | neu | der Code war im Profil fest gesetzt | `105059-0`und`664-3`, beide nominal | Die allgemeine Mikroskopie nimmt einen Code an, der die Färbung trägt | `105059-0`bleibt empfohlen |
...
| `mii-vs-mikrobio-empfindlichkeit-methode-snomed` | neu | `Observation.method`war im Empfindlichkeitsprofil ungebunden | Die fünf Verfahren, die das Workbook nennt: Bouillondilution, Agardiffusion, Gradientenstreifen, manuelle und automatisierte MHK-Mikrodilution | Methodencodes sind beschränkt, wo sie frei waren | Das Whitepaper nennt für dieselben fünf Verfahren Prozedur-Konzepte; dieses Modul bleibt bei Qualifier Values, wie seine übrigen neun Methodenlisten. Zurückgemeldet |
...
| `mii-vs-mikrobio-morphologie-ergebnis-snomed` | inhaltlich aktualisiert | 15 Codes, alle`(finding)`, alle angeordneten Formen | 27 Codes, nach Achse gruppiert: angeordnete Formen als`(finding)`, die nackten Morphologien als`(organism)`, weil SNOMED sie nur dort führt, Pilzmorphologie, sowie`No organisms seen`/`No cells seen`/`Clue cells present` | Die häufigsten Grambefunde werden abbildbar; von den bisherigen 15 Codes verwendete ein realer Laborkatalog drei | Die vier nackten bakteriellen Morphologien sind Nachkommen der Bacteria domain und damit auch gültige Werte der allgemeinen Bestimmung. Die Granularität steht im Profil oder im Untersuchungscode, nie in`value[x]`allein |
| `mii-vs-mikrobio-antigen-antikoerper-quantitativ-einheiten-ucum` | inhaltlich aktualisiert | acht Einheiten, mL und L gemischt ohne Vorrang | zehn Einheiten in drei Gruppen;`ng/L`und`pg/L`ergänzt, Liter als bevorzugter Nenner markiert | Alle bisherigen Einheiten bleiben gültig | Das europäische Whitepaper empfiehlt für die Serologie den Liter;`[IU]/mL`bleibt zulässig als international verbreitete Schreibweise |
| `mii-vs-mikrobio-spezifische-kultur-tests-loinc` | inhaltlich aktualisiert | nur Filter | vier Ausschlüsse:`44851-4`,`44852-2`,`44854-8`,`44855-5` | Die durchnummerierten Mykobakterien-Codes verlassen die Menge | Das Workbook markiert sie mit „Remove"; gemessen sind alle vier noch`STATUS = ACTIVE`, der Statusfilter wirft sie also nicht heraus |
| `mii-vs-mikrobio-spezifische-bestimmung-methode-snomed` | inhaltlich aktualisiert | zehn Verfahren | elf; ergänzt`702675006`sondenbasierte Zielamplifikation | Die häufigste moderne gezielte PCR wird kodierbar | Neben`258066000`, nicht statt dessen: Nicht jede Amplifikation ist sondenbasiert |
...
| `mii-vs-mikrobio-empfindlichkeit-phenotyp-loinc` | inhaltlich aktualisiert | nur Filter | zusätzlich`106843-6`und`103673-0`, aufgezählt | Zwei echte Empfindlichkeitstests gelangen in die Menge | Beide tragen`PROPERTY``Susc`, liegen aber in`CLASS`MICRO und DRUG/TOX; das Workbook bittet Regenstrief selbst um Korrektur |
...
| `mii-lm-mikrobio-untersuchungsarten` | inhaltlich aktualisiert | ein`Mikroskopie`-Element;`Empfindlichkeit.Ergebnis`als`Quantity`;`BarlettScore` | `AllgemeineMikroskopie`und`SpezifischeMikroskopie`;`Empfindlichkeit.Ergebnis[x]`als`Quantity`oder`CodeableConcept`;`BartlettScore` | Die Elementnamen des logischen Modells ändern sich | Das Modell spiegelt die Profile; die Änderung folgt der Mikroskopie-Aufteilung, dem geweiteten Empfindlichkeitsergebnis und der Bartlett-Korrektur |
...
| `mii-cps-mikrobio-metadata` | inhaltlich aktualisiert | 19 unterstützte Observation-Profile | 20;`mii-pr-mikrobio-spezifische-mikroskopie`ergänzt,`mii-pr-mikrobio-barlett-score`zu`-bartlett-score`korrigiert | Server, die Konformität erklären, führen das neue Profil | Die Profilliste anpassen |
...
* Sechs neue Beispiele, jedes für einen Fall, der vorher nicht abbildbar war: `mii-exa-mikrobio-allgemeine-kultur-methodenneutral` (der methodenneutrale Kulturcode mit seiner Pflichtmethode), `mii-exa-mikrobio-mikroskopie-gram-kokken-wenig` (Morphologie plus Menge als Komponente), `mii-exa-mikrobio-spezifische-mikroskopie-auramin-negativ` und `-kinyoun-zweiplus` (säurefeste Stufen), `mii-exa-mikrobio-spezifische-mikroskopie-leukozyten-gesichtsfeld` (eine Zählung je Gesichtsfeld) sowie `mii-exa-mikrobio-empfindlichkeit-nur-kategorie` (eine Kategorie ohne Messwert). Die Mikroskopie-Beispiele stammen aus einem realen Laborkatalog.
* Sechs Ballotfragen stehen im Leitfaden, jede auf der Seite, zu der sie gehört, und gesammelt auf der Startseite. Neue Abschnitte erklären Granularität gegen Achse, die Methodenachse, die Ausnahme der Serologie von der Specimen-Postkoordination, das Panel-Muster zur Gruppierung mehrerer Ergebnisse und den Testcode, den allgemeine Kultur und allgemeine Bestimmung teilen. Der Release-Hinweis nennt das Modul nicht mehr Alpha-Release.
...
* Ein siebtes Beispiel, `mii-exa-mikrobio-spezifische-bestimmung-influenza-positiv`, zeigt die Richtung von `derivedFrom`. Bisher trug `mii-exa-mikrobio-ct-wert` den Verweis selbst und zeigte auf einen negativen Nachweis — die Gegenrichtung zu dem, was `A.derivedFrom = B` bedeutet, und fachlich unmöglich, denn ein Ct von 21,3 ist ein starkes Positiv. Der Verweis sitzt jetzt am Nachweis, von dem die Ableitung ausgeht, und die beiden Beispiele teilen ein Codepaar, das sich nur in Property und Scale unterscheidet.
...
| Artefakt (Canonical-URL) | Änderungstyp | Vorher (falls relevant) | Nachher | Implementierungsauswirkung | Migrationshinweis | |————-|————–|————————–|———|—————————-|——————-| | `mii-pr-mikrobio-kultur-nachweis` | entfernt/ersetzt | aktiv bis Vorgängerversion | ersetzt durch neues 2027-Profil für allgemeine Kultur | Profil wird im 2027-Modell nicht mehr veröffentlicht | Auf neue 2027-Kulturstruktur umstellen | | `mii-pr-mikrobio-molekulare-diagnostik` | entfernt/ersetzt | aktiv bis Vorgängerversion | ersetzt durch neues 2027-Profil für spezifische Bestimmung | Profil wird im 2027-Modell nicht mehr veröffentlicht | Auf neue 2027-Bestimmungsstruktur umstellen | | `mii-pr-mikrobio-resistenzgene` | entfernt/ersetzt | aktiv bis Vorgängerversion | in zusammengeführtes 2027-Profil für Resistenzmechanismen/Determinanten aufgegangen | Profil wird im 2027-Modell nicht mehr veröffentlicht | Frühere Component-Angaben zum Gennamen (`component[NamedesGens*]`) auf präkoordinierte `Observation.code`-Codierung im Zielprofil mappen; Nachweis weiterhin über `valueCodeableConcept` | | `mii-pr-mikrobio-resistenzmutation` | entfernt/ersetzt | aktiv bis Vorgängerversion | in zusammengeführtes 2027-Profil für Resistenzmechanismen/Determinanten aufgegangen | Profil wird im 2027-Modell nicht mehr veröffentlicht | Frühere Component-Angaben zur Resistenzmutation (`component[MicroorganismResistanceMutation]`) auf präkoordinierte `Observation.code`-Codierung im Zielprofil mappen; Nachweis weiterhin über `valueCodeableConcept` | | `mii-pr-mikrobio-serologie-immunologie` | entfernt/ersetzt | aktiv bis Vorgängerversion | in 2027-Profil Spezfische Bestimmung überführt, Komponenten wurden in eigene Profile überführt | Profil wird im 2027-Modell nicht mehr veröffentlicht | Mapping in 2027-Profile überführen |
