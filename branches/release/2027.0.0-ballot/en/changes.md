# Changelog - MII Implementation Guide Microbiology v2027.0.0-ballot2
...
**No content change.** Every artefact is identical to `2027.0.0-ballot` apart from the version it carries. Use this version; the packaging of the previous one was faulty in two ways.

* The published package contained **31 artefacts that this release cycle had retired** — five profiles, five examples and twenty value sets, all still at version `2025.0.2` — because the publication path baked a project state rather than this repository. One of them collided with a current artefact: a second CapabilityStatement on the canonical `…/CapabilityStatement/metadata`, at `2025.0.2`, beside the current one. Which of the two a tool uses would have been decided by its load order.
* `mii-pr-mikrobio-probe` shipped **without a snapshot**, because the biobank package its parent lives in was not resolvable during that build.

Both causes sat in the publication path, not in the module. The artefacts of `2027.0.0-ballot` were correct; what was published under that name was not.
...
* Targeted detection stays in one profile whatever the technique - culture, molecular, immunological or biochemical. The question and the type of its answer are the same: is the named target there, `Detected` or `Not detected`. The culture is stated by the test code, whose method axis carries `Organism specific culture`, so it is not repeated in the result. The HL7 EU Lab Semantic Workgroup keeps targeted culture separate and binds growth codes there; this guide deviates and asks about it as ballot question 6.
...
* Microscopy was split into an open and a targeted half, as culture and determination already were. The distinction is the question: general microscopy answers with the morphological group seen, targeted microscopy with the detection of an object its test code names. How much was seen is a component in both, never the value - the targeted codes are `PrThr` terms and ask whether the object is there.
...
| `mii-pr-mikrobio-probe` | new | - | Specimen profile derived from`MII_PR_Biobank_Specimen_Bioprobe_Core`; keeps the inherited specimen type binding and documents the microbiological expectations on`type`,`collection.collected[x]`and`parent` | `Observation.specimen`now has a target profile; a Specimen must satisfy the biobank base profile, which makes`status`,`type`,`subject`,`collection`and`collection.collected[x]`mandatory | Add`collection.collected[x]`to existing Specimen resources. The storage temperature extension below`Specimen.processing`is no longer required: the base profile relaxed it to`0..1`in`2027.0.0-ballot` |
| `mii-pr-mikrobio-spezifische-mikroskopie` | new | previously not representable | Targeted microscopy: an ordinal LOINC code names the object and the stain,`value[x]`is`Detected`/`Not detected`,`component[menge]`carries the amount as a grade, a count per high power field or a range of such a count, and`extension[faerbung]`states the stain | Acid-fast stains, host cell counts and clue cells become codeable; they had no profile before | Move such findings here from general microscopy, where the amount had to be forced into`interpretation`. The value answers the detection question the code asks; put the grade in the component |
| `mii-pr-mikrobio-mikroskopie` | content updated | `code`fixed to`105059-0`; no component; title "Mikroskopie" | `code`from`mii-vs-mikrobio-allgemeine-mikroskopie-tests-loinc`(extensible), which also admits`664-3`; new`component[menge]`for the amount, as a grade or as a count per high power field, its code a pattern of system and code alone and its units bound; the stain moves into`extension[faerbung]`and`method`keeps the microscopy technique; title "Allgemeine Mikroskopie" | The fixed code is no longer enforced; a morphology and its amount can be sent together | `105059-0`remains preferred. Move stain codes out of`Observation.method`into`extension[faerbung]`, taking the procedure code instead of the qualifier value -`62777006`for`708061008` |
...
| `mii-vs-mikrobio-mikroskopie-semiquantitativ-snomed` | new | - | Seven grades on two SNOMED axes: presence findings`441614007`,`441517005`,`441521003`and the degree descriptors`Few`,`Moderate number`,`Numerous`,`Scanty` | Amount value set of the component in both microscopy profiles | The two axes are not interchangeable; which family the European model prefers is still open. Presence codes are not in it:`Present`,`Absent`,`None`and`No organisms seen`answer the detection question and belong in`value[x]` |
| `mii-vs-mikrobio-mikroskopie-gesichtsfeld-einheiten-ucum` | new | the counted form of the amount had no unit constraint at all | `/[HPF]`and`/[LPF]`, extensible, bound on the amount component's`valueQuantity.code`and on both`valueRange`bounds | A count per field is constrained to the field-of-view units, as every other quantitative statement of this module is | Both codes validate against UCUM; the binding is extensible, so a further unit stays possible |
| `mii-vs-mikrobio-aviditaet-ergebnis` | content updated | `L`,`I`,`H`from`v3-ObservationInterpretation` | `L`and`H`unchanged,`I`replaced by`280414007`Equivocal result, the code this guide already uses for a borderline finding in culture and targeted determination | `I`was not merely imprecise but wrong: THO subsumes it under the susceptibility branch and defines it as a bacterial strain inhibited by an antimicrobial agent with uncertain therapeutic effect, citing CLSI.`L`and`H`stay because the parent binds`interpretation`extensibly to a list that holds them, and an extensible binding demands the code from the list wherever one applies; for the middle the list offers none, which is what admits an outside code | Map`I`to`280414007`. Laboratories report all three categories, so the middle one stays |
...
| dependencies | content updated | `…kerndatensatz.laborbefund``2027.0.0-ballot.rc4`,`…kerndatensatz.biobank``2027.0.0-ballot.rc2` | both at the final`2027.0.0-ballot` | The laboratory parent is differential-identical to rc4, measured on the registry tarballs. The biobank release relaxes`Specimen.processing.extension:temperaturbedingungen`from`1..1`to`0..1`and moves the requirement to the storage slice | Nothing to migrate. A Specimen that carried a storage temperature under a processing step only to satisfy the old requirement can drop it |
...
| `mii-lm-mikrobio-untersuchungsarten` | new | - | The domain variants, derived from`mii-lm-mikrobio-untersuchung`, with`AllgemeineMikroskopie`and`SpezifischeMikroskopie`as separate variants, both carrying a`Menge[x]`element beside the result,`Empfindlichkeit.Ergebnis[x]`as`Quantity`or`CodeableConcept`and`BartlettScore`in the corrected spelling | New artefact in the information model | Map variant-specific elements against this |
...
* New examples, each documenting a case that was not representable before: [mii-exa-mikrobio-allgemeine-kultur-methodenneutral](Observation-mii-exa-mikrobio-allgemeine-kultur-methodenneutral.md) (the method-neutral culture code with its mandatory method), [mii-exa-mikrobio-mikroskopie-gram-kokken-wenig](Observation-mii-exa-mikrobio-mikroskopie-gram-kokken-wenig.md) (morphology plus amount as a component), [mii-exa-mikrobio-spezifische-mikroskopie-auramin-negativ](Observation-mii-exa-mikrobio-spezifische-mikroskopie-auramin-negativ.md) and [mii-exa-mikrobio-spezifische-mikroskopie-kinyoun-zweiplus](Observation-mii-exa-mikrobio-spezifische-mikroskopie-kinyoun-zweiplus.md) (acid-fast grades), [mii-exa-mikrobio-spezifische-mikroskopie-leukozyten-gesichtsfeld](Observation-mii-exa-mikrobio-spezifische-mikroskopie-leukozyten-gesichtsfeld.md) (a count per high power field), [mii-exa-mikrobio-empfindlichkeit-nur-kategorie](Observation-mii-exa-mikrobio-empfindlichkeit-nur-kategorie.md) (a category without a measurement) and [mii-exa-mikrobio-spezifische-bestimmung-influenza-positiv](Observation-mii-exa-mikrobio-spezifische-bestimmung-influenza-positiv.md), which shows the direction of `derivedFrom`: the detection points at the Ct value it was read from, where the reference previously sat on [mii-exa-mikrobio-ct-wert](Observation-mii-exa-mikrobio-ct-wert.md) and ran the other way. The microscopy examples are taken from a real laboratory result catalogue.
* The examples now share supporting resources instead of inlining them: [mii-exa-mikrobio-patient](Patient-mii-exa-mikrobio-patient.md), [mii-exa-mikrobio-labor](Organization-mii-exa-mikrobio-labor.md) as the performing laboratory, and [mii-exa-mikrobio-probe](Specimen-mii-exa-mikrobio-probe.md) with [mii-exa-mikrobio-probe-katheterspitze](Specimen-mii-exa-mikrobio-probe-katheterspitze.md) as specimens against the new Specimen profile.
...
* [mii-exa-mikrobio-spezifische-kultur](Observation-mii-exa-mikrobio-spezifische-kultur.md) reports `Detected` instead of `Organism growth`: a targeted culture answers whether the named target is there, and the culture itself is already stated by the test code.
...
* `mii-exa-mikrobio-mre-klasse` is dropped. New are two examples for the resistance category status, [mii-exa-mikrobio-resistenzkategorie-vre-positiv](Observation-mii-exa-mikrobio-resistenzkategorie-vre-positiv.md) and [mii-exa-mikrobio-resistenzkategorie-vre-negativ](Observation-mii-exa-mikrobio-resistenzkategorie-vre-negativ.md), representative of all categories; the negative examples [mii-exa-mikrobio-spezifische-bestimmung-vre-negativ](Observation-mii-exa-mikrobio-spezifische-bestimmung-vre-negativ.md) and [mii-exa-mikrobio-spezifische-kultur-vre-negativ](Observation-mii-exa-mikrobio-spezifische-kultur-vre-negativ.md); [mii-exa-mikrobio-mrgn-klasse-negativ](Observation-mii-exa-mikrobio-mrgn-klasse-negativ.md) for `keine-mrgn-klasse`; and the three-part chain [mii-exa-mikrobio-workflow-vre-01-kultur](Observation-mii-exa-mikrobio-workflow-vre-01-kultur.md) → [mii-exa-mikrobio-workflow-vre-02-identifikation](Observation-mii-exa-mikrobio-workflow-vre-02-identifikation.md) → [mii-exa-mikrobio-workflow-vre-03-empfindlichkeit](Observation-mii-exa-mikrobio-workflow-vre-03-empfindlichkeit.md) as the basis of the derivation.
