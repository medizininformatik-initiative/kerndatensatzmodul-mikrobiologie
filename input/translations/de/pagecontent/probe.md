...
ableitet — einem Basisprofil, das genau dafür geschrieben ist, dass Module davon
ableiten statt es direkt zu implementieren. `Observation.specimen` jedes
Untersuchungsprofils verweist darauf.
...
- **Dauer** — `Specimen.processing.time[x]` als `Period`. `Specimen.processing`
  ...
  Entnahme), Start und Ende sind also die FHIR-eigene Form. Genau das verlangt
  das europäische Datenmodell, wenn es zu seinem Kandidaten-Code `80581-2`
  vermerkt „mandates for start/enddate" — mit einem `Period` ist die Anforderung
...
Bebrütung nicht ab, solange die Antwort fehlt.
