...
| `mii-vs-mikrobio-spezifische-kultur-tests-loinc` | content updated | filter only | four excludes: `44851-4`, `44852-2`, `44854-8`, `44855-5` | The numbered Mycobacterium codes leave the set | The HL7 EU Lab Semantic Workgroup flags them "Remove"; measured, all four are still `STATUS = ACTIVE`, so the status filter does not drop them |
...
- A seventh example, `mii-exa-mikrobio-spezifische-bestimmung-influenza-positiv`, shows the direction of `derivedFrom`: the detection points at the Ct value it was read from. The reference previously sat on `mii-exa-mikrobio-ct-wert` and ran the other way.
