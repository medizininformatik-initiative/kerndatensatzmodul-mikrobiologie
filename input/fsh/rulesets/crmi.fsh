...
// MIGRATION: the module author's contact address is not declared anywhere in the
// source, so this rule is COMMENTED OUT rather than shipped with a placeholder or
// an invented address (spec guardrail 3). Uncomment it once the address is known.
// * ^extension[=].valueContactDetail.telecom[=].value = "name@example.org"   // ① OPEN
...
* extension[+].url = "http://hl7.org/fhir/StructureDefinition/artifact-author"
* extension[=].valueContactDetail.telecom[+].system = #email
// MIGRATION: see the note on the caret-path flavour above — ① OPEN.
// * extension[=].valueContactDetail.telecom[=].value = "name@example.org"
* extension[+].url = "http://hl7.org/fhir/StructureDefinition/artifact-editor"
* extension[=].valueContactDetail.name = "Taskforce Core Data Set"
* extension[+].url = "http://hl7.org/fhir/StructureDefinition/artifact-reviewer"
* extension[=].valueContactDetail.name = "Interoperability Working Group"
