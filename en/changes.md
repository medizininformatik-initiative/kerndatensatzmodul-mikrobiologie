...
* The diagnostic chain is now documented where it is used. `triggeredBy`, `derivedFrom` and `hasMember` were described on the profile-selection page alone; every profile that takes part in a chain now states its own position in it, susceptibility carries the panel pattern for an antibiogram, and the report profile states that it bundles the steps while the Observations themselves carry the relationships between them.
...
* A seventh example, `mii-exa-mikrobio-spezifische-bestimmung-influenza-positiv`, shows the direction of `derivedFrom`. `mii-exa-mikrobio-ct-wert` carried the reference itself and pointed at a negative detection - the reverse of what `A.derivedFrom = B` means, and impossible in domain terms, because a Ct of 21.3 is a strong positive. The reference now sits on the detection, which is where the derivation runs from, and the two examples share a code pair that differs only in property and scale.
