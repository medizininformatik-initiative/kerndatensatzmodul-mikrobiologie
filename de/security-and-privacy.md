# Sicherheit und Datenschutz - MII Implementation Guide Microbiology v2027.0.0-ballot.rc1
...
**Ein Befund ist sensibel ohne Diagnose.** Der Untersuchungscode benennt das Ziel: Ein HIV-, Tuberkulose- oder Hepatitis-Ergebnis ist aus `Observation.code` ablesbar, auch wenn keine `Condition` existiert. Zugriffsregeln, die an Diagnosen anknüpfen, sehen ihn nicht.

**Der Kolonisationsstatus überdauert den Behandlungsfall.** MRSA, VRE, LRE, LVRE und die MRGN-Klassen beschreiben die Person statt des Kontakts und steuern Isolierungs- und Aufnahmeentscheidungen — modelliert sind sie als gewöhnliche Observations, denen dieses Gewicht nicht anzusehen ist.

**Die IfSG-Meldepflicht läuft anderswo.** Ein Teil der hier abbildbaren Erreger ist nach dem Infektionsschutzgesetz meldepflichtig. Diese Pflicht wird über demis erfüllt; der Empfang dieser Ressourcen zu Forschungszwecken erfüllt sie weder noch ersetzt er sie. Dieses Modul definiert keinen Meldeworkflow und keinen Meldestatus.
